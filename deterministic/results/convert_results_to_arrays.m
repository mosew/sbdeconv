...
AUC_sq_errors = zeros(bsize);
peak_time_sq_errors = zeros(bsize);
peak_height_sq_errors = zeros(bsize);
...
for para = 1:bsize(1)
    for test = 1:bsize(2)                    
        ...
        AUC_sq_errors(para,test) = s.AUC_sq_error;
        peak_time_sq_errors(para,test) = s.peak_time_sq_error;
        peak_height_sq_errors(para,test) = s.peak_height_sq_error;
...
logAUC_sq_errors = log(AUC_sq_errors);
logAUC_MSE = mean(AUC_sq_errors,2);
...
AUC_MSE = mean(AUC_sq_errors,2);
peak_time_MSE = mean(peak_time_sq_errors,2);
peak_height_MSE = mean(peak_height_sq_errors,2);

L2_MSE_sd = std(L2_errors,0,2);
logL2_sd = std(logL2_errors,0,2);
Linf_sd = std(Linf_errors,0,2);
AUC_MSE_sd = std(AUC_sq_errors.^2,0,2);
peak_time_MSE_sd = std(peak_time_sq_errors.^2,0,2);
peak_height_MSE_sd = std(peak_height_sq_errors.^2,0,2);
