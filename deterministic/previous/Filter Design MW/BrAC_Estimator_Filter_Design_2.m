function [q_1_out,q_2_out,q_3_out,r1_r2_h,Est_TAC_q_in,Est_TAC_q_out] = ...
    BrAC_Estimator_Filter_Design_2(q_1_in,q_2_in,q_3_in,BrAC,TAC)

%  System Requirements: Standard MATLAB Version 7 or Later including 
%  Optimization Toolbox

%  Input:  q_1,q_2,q_3: Model Parameters
%          BrAC: First Column: Time in Hours
...
%           2-Column Array Est_TAC_q_in:  First Column: Time in Hours
%                                         
%                                         Second Column: Modeled TAC 
%                                          (using q_in and BrAC) in % Alcohol
%
%           2-Column Array Est_TAC_q_out:  First Column: Time in Hours
%                                         
%                                         Second Column: Modeled TAC 
%                                          (using q_out and e_BrAC) in % Alcohol
%
...
CD_beg = 0;
CD_end = floor(max(max(BrAC_TAC_Data_BrAC_time),max(BrAC_TAC_Data_TAC_time)));

BrAC_TAC_time = CD_beg:sampling_interval:CD_end;

I_BrAC = find(((BrAC_TAC_time - floor(max(BrAC_TAC_Data_BrAC_time)))>= 0),1);
r_BrAC_data = [max(spline(BrAC_TAC_Data_BrAC_time,BrAC_TAC_Data_BrAC_data,...
    BrAC_TAC_time(1:I_BrAC)),0),zeros(1,length(BrAC_TAC_time)-I_BrAC)];

I_TAC = find(((BrAC_TAC_time - floor(max(BrAC_TAC_Data_TAC_time)))>= 0),1);
r_TAC_data = [max(1000*spline(BrAC_TAC_Data_TAC_time,BrAC_TAC_Data_TAC_data,...
    BrAC_TAC_time(1:I_TAC)),0),zeros(1,length(BrAC_TAC_time)-I_TAC)];
...
if (t_TAC(end) > t_BrAC(end))
    ...
    data_BrAC = [data_BrAC;zeros(n_BrAC,1)];
...
if (t_TAC(end) < t_BrAC(end))
    ...
    data_TAC = [data_TAC;zeros(n_TAC,1)];
...
if (norm([q_1_in,q_2_in,q_3_in])~= 0)
    
    
    if (Parabolic)
      
      [y_hat] = TS_J_eval_3_EXP_PARABOLIC_G_FD_1([q_1_in,q_2_in]);
                        
    else
      
      [y_hat] = TS_J_eval_3_EXP_HYPERBOLIC_G_FD_1([q_1_in,q_2_in,q_3_in])
      
    end
    
end

Est_TAC_q_in = [Training_Data.t_tau',y_hat'];
...
OPTIONS = optimset('Display','off','MaxIter',5000,'MaxFunEvals',3000,'TolFun',10^-8,'TolX',10^-6);
warning off
